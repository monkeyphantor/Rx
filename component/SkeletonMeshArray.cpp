...
namespace Rx
{
    namespace Component
    {
        void SkeletonMeshArray::addMesh(const std::string& name,
            const std::vector<Vertex::Skeleton>& vertices, 
            const std::vector<uint32_t>& indices)
        {
            if (meshNameToCommand.find(name) != meshNameToCommand.end())
            {
                RX_LOGE("SkeletonMeshArray", "addMesh", ("SkeletonMesh with name " + name + " already exists.").c_str());
            }
